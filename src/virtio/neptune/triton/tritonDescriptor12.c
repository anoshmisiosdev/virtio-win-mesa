                          ...
                          D3D12DDI_CPU_DESCRIPTOR_HANDLE DestDescriptor)
{
    ...
    const BOOL msaa = r->Desc.SampleDesc.Count > 1;
    ...
    ID3D12Device_CreateRenderTargetView(p->pDev, r->pResource, &vd, dst);
}
                          ...
                          D3D12DDI_CPU_DESCRIPTOR_HANDLE DestDescriptor)
{
    ...
    PTRITON12_RESOURCE r =
        pArgs ? (PTRITON12_RESOURCE)pArgs->hDrvResource.pDrvPrivate : NULL;
    if (!p || !p->pDev || !pArgs || !r || !r->pResource)
        return;
    D3D12_CPU_DESCRIPTOR_HANDLE dst;
    dst.ptr = (SIZE_T)DestDescriptor.ptr;
    ...
    ID3D12Device_CreateDepthStencilView(p->pDev, r->pResource, &vd, dst);
}
