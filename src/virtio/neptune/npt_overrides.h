 ...
 * the ExecuteCommandLists that referenced `list_wrapper`, so a Reset
 * issued immediately after submission (legal D3D12) waits that ring
 * forward before its own async dispatch.  False when the wrapper has
 * no list aux (QI alias / OOM). */
bool
