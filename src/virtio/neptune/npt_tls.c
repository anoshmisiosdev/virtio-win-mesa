 ...
 * NPT_TLS_RING_KB overrides at runtime. */
...
npt_tls_ring_buffer_size(void)
{
   static unsigned sz;
   if (!sz) {
      const char *e = getenv("NPT_TLS_RING_KB");
      unsigned kb = e && e[0] ? (unsigned)strtoul(e, NULL, 10) : 0;
      sz = kb ? kb * 1024u : NPT_TLS_RING_BUFFER_SIZE_DEFAULT;
   }
   return sz;
}
