...
{
   const UINT64 value = (UINT64)InterlockedIncrement64(&gpu->drain_value);
   ...
   if (!NT_SUCCESS(status)) {
      npt_log("virtgpu drain: SignalFromGpu failed 0x%lx", status);
      return false;
   }
...
npt_renderer_create_virtgpu(void)
{
   ...
   npt_env_init();
   if (gpu->wddm2 && !NPT_DEBUG(NO_WDDM2_SUBMIT) &&
       virtgpu_create_context_virtual(gpu)) {
