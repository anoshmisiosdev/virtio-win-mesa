 ...
 * it after the fence retires (post-fire, so the D3D library's stored
 * eventfd number stays valid until it has been written). */
...
struct npt_cmd_arm_event_fence {
   ...
   uint64_t event_token;
   uint32_t ring_idx;
   uint32_t flags; /* NPT_EVENT_ARM_FLAG_* (was pad; old guests send 0) */
};
